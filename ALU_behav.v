...
module ALU_behav( ADin, BDin, ALU_ctr, Result, Overflow, Carry_in, Carry_out, Zero,shAmt ); 
   ...
   always @(ALU_ctr or ADin or BDin or Carry_in)
	 ...
	 `ADD:  begin
	  ...
	  `SUB:  begin
	   ...
	   `SRA: Result<=BDin>>>shAmt;
       `SRL: Result<=BDin>>shAmt;
       `SLL: Result<=BDin<<shAmt;
       `LWSW: Result<=ADin+BDin;
       `BE:  begin
	      {C6arry_out, tmp} = ADin - BDin;
          Overflow = ADin[n-1] & ~BDin[n-1] & ~tmp[n-1] 
          | ~ADin[n-1] & BDin[n-1] & tmp[n-1];
          $display("SLT:- [%d] tmp = %d [%b]; Cout=%b, Ovf=%b; A=%d, B=%d",
            $time, tmp, tmp, Carry_out, Overflow, ADin, BDin );
